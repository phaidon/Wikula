<div class="wikula_block_recentchanges">
    ...
    <a class="z-icon-es-rss" href="{modurl modname='Wikula' type='user' func='recentchangesxml' theme='rss'}" title="RSS">
        {gt text="RSS Feed"}
    </a>
    ...
    {if $pagelist}
        ...
        {foreach from=$pagelist key='date' item='pages'}
            <ul class="recentchanges">
                {foreach from=$pages item='page'}
                    <li>
                        <a href="{modurl modname='Wikula' type='user' func='show' tag=$page.tag|urlencode}" title="{$page.tag|safehtml}">{$page.tag|safehtml}</a>
        ...
        {foreachelse}
            <ul>
                <li>{gt text='There are no recent changes'}</li>
            </ul>
        {/foreach}
    {/if}
