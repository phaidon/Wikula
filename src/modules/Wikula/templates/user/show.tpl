...
<div id="wikula">
    ...
    {modurl modname='Wikula' type='user' func='main' tag=$showpage.tag assign='showpageurl'}
    {modurl modname='Wikula' type='user' func='main' tag=$showpage.tag func='revisions' assign='revisionsurl'}
    <div class="revisioninfo z-clearfix">
        <h4>{gt text='Revision [%s]' tag1=$showpage.id}</h4>
        <p>
            {gt text='This is a past revision of <a href="%2$s">%1$s</a> made by %3$s on <a class="datetime" href="%4$s">%5$s</a>' tag1=$showpage.tag|safehtml tag2=$showpageurl|safehtml tag3=$showpage.user|safehtml tag4=$revisionsurl|safehtml tag5=$time|safehtml}
        </p>
        <form class="z-floatleft" action="{modurl modname='Wikula' type='user' func='main' tag=$tag|urlencode}" method="post" enctype="application/x-www-form-urlencoded">
            <div>
