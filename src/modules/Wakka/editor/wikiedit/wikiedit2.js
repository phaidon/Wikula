...
// this.addButton("quote","quote","'\\n<[',']>\\n',2");
 ...
 if (maAvailable) {
 this.addButton("attach","Media","","document.getElementById('" + this.id + "')._owner.attachMedia");
 }
...
WikiEdit.prototype.createImage = function (isAlt)
{
  ...
  if (!n.test(this.sel)) {
    if (!isAlt) {
     ...
     this.sel = '{{image url="'+lnk+'" alt="'+sl+'" class="'+align+'"}}';
    };
...
WikiEdit.prototype.attachMedia = function (isAlt)
{
  var t = this.area;
  t.focus();
  ...
  if (!n.test(this.sel)) {
    if (!isAlt) {
     id = prompt("Media ID:", this.sel);
     if (id==null) return false;
     title = prompt("Title:", this.sel);
     if (title==null) title = "";
     w_class = prompt("CSS class:", this.sel);
     if (w_class==null) wclass = "";
     group = prompt("Lightbox group (empty if none):", this.sel);
     if (group==null) group = "";

     this.sel = '{{mediaattach fid="'+id+'" title="'+title+'" class="'+w_class+'" group="'+group+'"}}';
    };
